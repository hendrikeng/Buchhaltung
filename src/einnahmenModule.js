/**
 * einnahmenModule.js - Modul für die Verwaltung von Einnahmen
 *
 * Verwaltet alle Funktionen für Einnahmen und Rechnungen
 */

const EinnahmenModule = (function() {
     ...
     * @param {Date} datum - Rechnungsdatum
     * @param {number} kundenNummer - Kundennummer
     * @returns {string} Generierte Rechnungsnummer
     */
    function generateRechnungsnummer(datum, kundenNummer) {
        const date = new Date(datum);
        const year = date.getFullYear();
        const month = String(date.getMonth() + 1).padStart(2, '0');
        const day = String(date.getDate()).padStart(2, '0');

        return `RE-${year}-${month}-${day}-${kundenNummer}`;
    }

    /**
     * Validiert die Einnahmen-Daten und aktualisiert Berechnungen
     * @param {Object} sheet - Sheet-Objekt (Einnahmen)
     */
    function validateAndUpdate(sheet) {
        ...
        for (let i = 1; i < data.length; i++) {
            ...
            if (!row[CONFIG.SYSTEM.EINNAHMEN_COLS.DATUM] && !row[CONFIG.SYSTEM.EINNAHMEN_COLS.RECHNUNGSNUMMER]) {
                continue;
            }
            ...
            if (!row[CONFIG.SYSTEM.EINNAHMEN_COLS.QUARTAL] && row[CONFIG.SYSTEM.EINNAHMEN_COLS.DATUM]) {
                const datum = new Date(row[CONFIG.SYSTEM.EINNAHMEN_COLS.DATUM]);
                const quartal = getQuartal(datum);

                updates.push({
                    row: rowNum,
                    col: CONFIG.SYSTEM.EINNAHMEN_COLS.QUARTAL + 1,
                    value: quartal
                });
            }
            ...
            if (Math.abs(calculatedMwSt - mwstBetrag) > 0.01) {
                updates.push({
                    row: rowNum,
                    col: CONFIG.SYSTEM.EINNAHMEN_COLS.MWST_BETRAG + 1,
            ...
            const calculatedBrutto = netto + (parseFloat(row[CONFIG.SYSTEM.EINNAHMEN_COLS.MWST_BETRAG]) || calculatedMwSt);

            if (Math.abs(calculatedBrutto - brutto) > 0.01) {
                updates.push({
                    row: rowNum,
                    col: CONFIG.SYSTEM.EINNAHMEN_COLS.BRUTTO + 1,
            ...
            if (Math.abs(calculatedRestbetrag - restbetrag) > 0.01) {
                updates.push({
                    row: rowNum,
                    col: CONFIG.SYSTEM.EINNAHMEN_COLS.RESTBETRAG + 1,
            ...
            if (newStatus !== zahlungsstatus) {
                updates.push({
                    row: rowNum,
                    col: CONFIG.SYSTEM.EINNAHMEN_COLS.ZAHLUNGSSTATUS + 1,
                    value: newStatus
                });
            }
            ...
            updates.push({
                row: rowNum,
                col: CONFIG.SYSTEM.EINNAHMEN_COLS.LETZTE_AKTUALISIERUNG + 1,
                value: new Date()
            });
        }
    ...
    /**
     * Importiert eine Rechnung aus Google Drive und fügt sie ins Einnahmen-Sheet ein
    ...
    function importInvoiceFromDrive(fileId) {
        try {
            ...
            const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.SYSTEM.SHEET_NAMES.EINNAHMEN);
            const data = sheet.getDataRange().getValues();

            // Nach dem Dateinamen oder der URL in den vorhandenen Einträgen suchen
            for (let i = 1; i < data.length; i++) {
                if (data[i][CONFIG.SYSTEM.EINNAHMEN_COLS.DATEINAME] === fileName ||
                    data[i][CONFIG.SYSTEM.EINNAHMEN_COLS.RECHNUNG_LINK] === fileUrl) {
                    return {
                        success: false,
            ...
            newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.DATUM] = new Date();
            newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.MWST_SATZ] = CONFIG.STEUER.MEHRWERTSTEUER_STANDARD;
            newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.QUARTAL] = getQuartal(new Date());
            newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.ZAHLUNGSSTATUS] = 'Offen';
            newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.DATEINAME] = fileName;
            newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.RECHNUNG_LINK] = fileUrl;
            newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.LETZTE_AKTUALISIERUNG] = new Date();

            // Rechnungsnummer aus Dateinamen extrahieren (falls vorhanden)
            const rechnungsNrMatch = fileName.match(/RE-\d{4}-\d{2}-\d{2}-\d+/);
            if (rechnungsNrMatch) {
                newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.RECHNUNGSNUMMER] = rechnungsNrMatch[0];
            }
            ...
            logFileImport('Einnahme', fileName, fileUrl);
    ...
    return {
        /**
         * Aktualisiert das Einnahmen-Sheet
         * - Berechnet fehlende Werte
         * - Aktualisiert Status
         * @returns {number} Anzahl der durchgeführten Updates
         */
        refreshEinnahmen: function() {
            const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.SYSTEM.SHEET_NAMES.EINNAHMEN);
            if (!sheet) {
                throw new Error(`Sheet "${CONFIG.SYSTEM.SHEET_NAMES.EINNAHMEN}" nicht gefunden`);
            }

            const updatesCount = validateAndUpdate(sheet);
            Logger.log(`Einnahmen aktualisiert: ${updatesCount} Änderungen`);
            ...
            if (result.success) {
                ...
                if (result.row) {
                    SpreadsheetApp.getActiveSpreadsheet()
                        .getSheetByName(CONFIG.SYSTEM.SHEET_NAMES.EINNAHMEN)
        ...
        createEinnahme: function(data) {
            try {
                ...
                if (!data.kunde) throw new Error('Kunde ist ein Pflichtfeld');
                ...
                if (!data.rechnungsnummer) {
                    data.rechnungsnummer = generateRechnungsnummer(data.datum, data.kunde.replace(/[^0-9]/g, ''));
                }
                ...
                // Daten setzen
                newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.DATUM] = new Date(data.datum);
                newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.RECHNUNGSNUMMER] = data.rechnungsnummer;
                newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.KATEGORIE] = data.kategorie || 'Erlöse aus Lieferungen und Leistungen';
                newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.KUNDE] = data.kunde;
                newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.NETTO] = parseFloat(data.netto);
                newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.MWST_SATZ] = parseFloat(data.mwstSatz);

                // Berechnete Felder
                const mwstBetrag = calculateMwSt(data.netto, data.mwstSatz);
                const brutto = parseFloat(data.netto) + mwstBetrag;

                newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.MWST_BETRAG] = mwstBetrag;
                newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.BRUTTO] = brutto;
                newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.BEZAHLT] = data.bezahlt || 0;
                newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.RESTBETRAG] = parseFloat(data.netto) - (data.bezahlt || 0);
                newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.QUARTAL] = getQuartal(data.datum);

                // Status
                if (data.bezahlt && parseFloat(data.bezahlt) >= parseFloat(data.netto)) {
                    newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.ZAHLUNGSSTATUS] = 'Bezahlt';
                    newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.ZAHLUNGSART] = data.zahlungsart || 'Überweisung';
                    newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.ZAHLUNGSDATUM] = data.zahlungsdatum || new Date();
                } else if (data.bezahlt && parseFloat(data.bezahlt) > 0) {
                    newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.ZAHLUNGSSTATUS] = 'Teilweise bezahlt';
                    newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.ZAHLUNGSART] = data.zahlungsart || 'Überweisung';
                    newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.ZAHLUNGSDATUM] = data.zahlungsdatum;
                } else {
                    newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.ZAHLUNGSSTATUS] = 'Offen';
                }

                // Metadaten
                if (data.dateiname) newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.DATEINAME] = data.dateiname;
                if (data.link) newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.RECHNUNG_LINK] = data.link;
                newRow[CONFIG.SYSTEM.EINNAHMEN_COLS.LETZTE_AKTUALISIERUNG] = new Date();
                ...
                return {
                    success: true,
                    message: `Einnahme "${data.rechnungsnummer}" erfolgreich erstellt`,
                    row: lastRow + 1,
                    rechnungsnummer: data.rechnungsnummer
                };
            } catch (e) {
                Logger.log(`Fehler beim Erstellen der Einnahme: ${e.message}`);
                return {
                    success: false,
        ...
        /**
         * Markiert eine Einnahme als bezahlt
        ...
        markAsPaid: function(rechnungsnummer, paymentData) {
            try {
                ...
                for (let i = 1; i < data.length; i++) {
                    if (data[i][CONFIG.SYSTEM.EINNAHMEN_COLS.RECHNUNGSNUMMER] === rechnungsnummer) {
                        rowIndex = i;
                        invoiceData = data[i];
                        break;
                    }
                ...
                if (invoiceData[CONFIG.SYSTEM.EINNAHMEN_COLS.ZAHLUNGSSTATUS] === 'Bezahlt') {
                    return {
                        success: false,
                ...
                const brutto = parseFloat(invoiceData[CONFIG.SYSTEM.EINNAHMEN_COLS.BRUTTO]) || 0;
                const bezahlt = parseFloat(paymentData.betrag || brutto);

                // Zeile aktualisieren (1-basierter Index)
                sheet.getRange(rowIndex + 1, CONFIG.SYSTEM.EINNAHMEN_COLS.BEZAHLT + 1).setValue(bezahlt);
                sheet.getRange(rowIndex + 1, CONFIG.SYSTEM.EINNAHMEN_COLS.ZAHLUNGSSTATUS + 1).setValue('Bezahlt');
                sheet.getRange(rowIndex + 1, CONFIG.SYSTEM.EINNAHMEN_COLS.ZAHLUNGSART + 1).setValue(paymentData.zahlungsart || 'Überweisung');
                sheet.getRange(rowIndex + 1, CONFIG.SYSTEM.EINNAHMEN_COLS.ZAHLUNGSDATUM + 1).setValue(paymentData.datum || new Date());
                sheet.getRange(rowIndex + 1, CONFIG.SYSTEM.EINNAHMEN_COLS.RESTBETRAG + 1).setValue(0);
                sheet.getRange(rowIndex + 1, CONFIG.SYSTEM.EINNAHMEN_COLS.LETZTE_AKTUALISIERUNG + 1).setValue(new Date());
