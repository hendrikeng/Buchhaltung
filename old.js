...
function updateEinnahmenAusgabenTab(sheet) {
    ...
    for (let i = 2; i <= lastRow; i++) {
        ...
        formulas9.push([`=(H${i}-I${i})/(1+VALUE(F${i}))`]); // Spalte J: Restbetrag netto
...
function processGuVRow(row, guvData, isIncome) {
    ...
    // Berechnung des tatsächlich gezahlten Netto-Betrags
    const gezahltNetto = Math.max(0, nettoRechnung - restbetragNetto);
